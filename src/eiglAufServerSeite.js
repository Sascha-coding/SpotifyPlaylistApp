...
class Spotifyy{
    ...
    getAccessToken = () => {
        ...
        let accessToken, tokenType, expiresIn, connected = false;
    ...
    exportPlaylist = async (playlist) => {
        ...
        let playlists = await fetch(`https://api.spotify.com/v1/users/${UserId}/playlists`,{
            method: "GET",
            headers: {
                Authorization: `Bearer ${accessToken}`,
        ...
        const existingPlaylist = await playlists.filter(item => playlist.spotifyId === item.id);
        let playlistId = existingPlaylist.length > 0 ? existingPlaylist[0].id : await this.createNewPlaylist(playlist); 
        ...
        if(existingPlaylist.length > 0){
            ...
            let tracksToAdd = await trackUris.filter(track => !existingUris.some(existingTrack => existingTrack === track));
            let tracksToRemove = await existingUris.map(track => trackUris.indexOf(track) === -1 ? track : null).filter(track => track !== null);
    ...
    addTracks = async (playlistId,trackUris,snapshot) =>{
        const accessToken = sessionStorage.getItem("accessToken");
        await fetch(`https://api.spotify.com/v1/playlists/${playlistId}/tracks?uris=${trackUris.join(",")}`,{
                method: "POST",
                headers: {
                    Authorization: `Bearer ${accessToken}`,
                    "Content-Type": "application/json",
    ...
    getTracks = async (playlist) => {
        const accessToken = sessionStorage.getItem("accessToken");
        const tracks = await fetch(`https://api.spotify.com/v1/playlists/${playlist.id}/tracks?&limit=100`,{
                method: "GET",
                headers: {
                    Authorization: `Bearer ${accessToken}`
                },
    ...
    getPlaylists = async () => {
        ...
        let result = await playlists.items.filter(playlist => playlist.tracks.total !== 0).map(async(playlist) => {
            ...
            for(let i = 0 ; i < playlist.tracks.total; i += 100){
                await fetch(`https://api.spotify.com/v1/playlists/${playlist.id}/tracks?offset=${i}&limit=100`,{
                method: "GET",
                headers: {
                    Authorization: `Bearer ${accessToken}`
                },
            ...
            playlistObj["playlistImage"] = playlistObj.playlistTracks.length > 0 ? playlistObj.playlistTracks[0].image : "";
